...
" These functions are used by the whproj plugin.
...
" Set b:whproj_comp_arg to the gzip argument to be used for compression, based on
...
fun s:set_compression(line)
  ...
  if l:cm == 8
    ...
    " max compression
    if l:xfl == 2
      let b:whproj_comp_arg = "-9"
    " min compression
    elseif l:xfl == 4
      let b:whproj_comp_arg = "-1"
    endif
  endif
endfun
...
fun whproj#read(cmd)
  ...
  " for whproj check current compression level and set b:whproj_comp_arg.
  silent! unlet b:whproj_comp_arg
  if a:cmd[0] == 'g'
  ...
  if !filereadable(tmp)
    ...
    echoerr "Error: Could not read uncompressed file" . tmpe
    let ok = 0
  else
    let ok = 1
...
fun whproj#write(cmd)
  ...
  if s:check(a:cmd)
    ...
    if rename(nm, nmt) == 0
      if exists("b:whproj_comp_arg")
	call system(a:cmd . " " . b:whproj_comp_arg . " -- " . s:escape(nmt))
      else
	call system(a:cmd . " -- " . s:escape(nmt))
      endif
      call rename(nmt . ".gz", nm)
    endif
  endif
endfun

" Before appending to compressed file: Uncompress file with "cmd"
fun whproj#appre(cmd)
  " don't do anything if the cmd is not supported
  if s:check(a:cmd)
    let nm = expand("<afile>")
